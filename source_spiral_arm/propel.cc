...
#include <iostream>
#include <sstream>
#include <cstring> //YO/20140221
#include <time.h> //YO/20141203
...
//Distribution primary_source_function_sub;
Distribution Work;
...
int Galprop::propel(Particle& particle) {
  ...
  int start=1;
  //double sigma =1. ;
  double spdistance; //SB20161111
  double newfunction; //SB20161111
  double fraction; //SB20161111
  double fraction1;
  double fraction2;
  double fraction3;
  ...
  particle.m_step_total_time =0;  //YO/20140611  
  ...
  if(strcmp(particle.name, "primary_TD_positron")!=0 &&
     strcmp(particle.name, "primary_TD_electron")!=0){ //YO/20140305 -> 20140408 changed
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz,ir)
    ...
    if(strcmp(particle.name,"primary_TD_positron")==0 ||
       strcmp(particle.name,"primary_TD_electron")==0){
      cout << " >>>> gen_TD_injection function started" << endl;
	...
	particle.m_dt = dt/year2sec; //YO/20140305
	
	if(galdef.check_num==305){
	  cerr << "propagating time = " << particle.m_step_total_time << "yr dt = " << particle.m_dt << "yr" << endl;
	} //YO/20140305

	particle.m_step_total_time += dt/year2sec; //YO/20140611
	
	//time dependent secondary source function //YO/20140220~20140305 -> 20140408 changed
	if(strcmp(particle.name, "primary_TD_positron")==0 ||
	   strcmp(particle.name, "primary_TD_electron")==0){
	  gen_TD_secondary_source(particle);
	 // total_source_function = particle.primary_source_function + particle.secondary_source_function;
	  total_source_function = particle.secondary_source_function; //SB20161611
	}
...
//	cout<<"\b\b\b\b\b"<<irept;   // \b=backspace
	...
	if(irept%galdef.timestep_diagnostics==0)
      ...
      particle.m_final_time = particle.m_step_total_time;

      if(galdef.check_num==305){
	cerr << "finally total time = " << particle.m_final_time << "yr" << endl;
      } //YO/20140325

      particle.m_step_total_time = 0; //initialization YO/20140409
  ...
  if (3 == particle.n_spatial_dimensions) {             // ==== 3D ====
    ...
    //primary_source_function_sub.init(particle.n_xgrid,particle.n_ygrid,particle.n_zgrid,particle.n_pgrid); //YO/20140612
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz,ix,iy)
    ...
    if(strcmp(particle.name,"primary_TD_positron")==0 ||
      ...
      //Calculate normalization factor for single source spectrum   YO/20141117
      cout << " generating " << particle.name << " source function for n_spatial_dimensions=" << gcr[0].n_spatial_dimensions << endl;
      int nor_num = galdef.TD_SNR_model;
      switch(nor_num){
      case 1:
	nor_SNR_burstlike(particle); //YO/20141117
	break;
	case 3: //ここではだめ？
	nor_SNR_exp_decay(particle); //YO/20141203
	break;
      }

      //Define single source coordinate   YO/20141203  //銀河座標と距離からだす

      cout << " defined coordinate of single source";
      double dist,sourceX,sourceY,sourceZ;
      double l = galdef.longitude*Pi/180; //deg
      double b = galdef.latitude*Pi/180;  //deg
      static int flag = 0;
      
      if(flag==0){
	if(galdef.random_coordinate==0){
	  dist = galdef.distance;
	  sourceX = 8.5-dist*cos(b)*cos(l);
	  sourceY = dist*cos(b)*sin(l);
	  sourceZ = dist*sin(b);
	}
	if(galdef.random_coordinate==1){
	  srand((unsigned) time(NULL));
	  int max = galdef.rand_max*100; //0.01 order
	  int min = galdef.rand_min*100;
	  dist = (double)(rand()%(max-min+1)+min)/100;
	  sourceX = 8.5-dist*cos(b)*cos(l);
	  sourceY = dist*cos(b)*sin(l);
	  sourceZ = dist*sin(b);
	}
	if(sourceX>0)
	  particle.m_sourceX = (double)(int)(sourceX*pow(10.0,1)+0.5)/pow(10.0,1); //0.1kpc精度になるよう四捨五入
	else particle.m_sourceX = (double)(int)(sourceX*pow(10.0,1)-0.5)/pow(10.0,1);
	if(sourceY>0){
	  double sourceYY = (double)(int)(sourceY*pow(10.0,1)+0.5)/pow(10.0,1);
	  particle.m_sourceY = -sourceYY;
	}
	else{
	  double sourceYY = (double)(int)(sourceY*pow(10.0,1)-0.5)/pow(10.0,1);
	  particle.m_sourceY = -sourceYY;
	...
	if(sourceZ>0)
	  particle.m_sourceZ = (double)(int)(sourceZ*pow(10.0,1)+0.5)/pow(10.0,1);
	else particle.m_sourceZ = (double)(int)(sourceZ*pow(10.0,1)-0.5)/pow(10.0,1);
	
	flag = 1;
      }
      cout << "= (" << particle.m_sourceX << "," << particle.m_sourceY << "," << particle.m_sourceZ << ")" << endl;

      //Define single source age YO/20141224
      cout << " defined age of single source : ";

      static int flag2 = 0;
      if(flag2 == 0){
	if(galdef.random_age==0){
	  particle.m_source_age = (int)((galdef.source_age + dist*kpc2cm/c/year2sec)/galdef.start_timestep+0.5)*galdef.start_timestep; //YO/20151013 consider ly 
	}
	if(galdef.random_age==1){
	  srand((unsigned) time(NULL));
	  int max = galdef.rant_max/(dt/year2sec); //dt[sec]
	  int min = galdef.rant_min/(dt/year2sec);
	  particle.m_source_age = (double)(rand()%(max-min+1)+min)*(dt/year2sec);
	}
	flag2 = 1;
      }
      cout << particle.m_source_age << endl;
	...
	//total calculation time  YO/20150112
	double dtt = galdef.start_timestep; 
	static int flagdtt = 0;
	if(galdef.random_age==0 && flagdtt==0){
	  particle.m_final_time = 0;
	  while(dtt > galdef.end_timestep*0.9999){
	    for(int i=0;i<galdef.timestep_repeat;i++){
	      particle.m_final_time += dtt;
	    }
	    dtt *= galdef.timestep_factor;
	  }
	  flagdtt = 1;
	}
	...
	*/
	...
	if(strcmp(particle.name, "primary_TD_positron")!=0 &&
	   strcmp(particle.name, "primary_TD_electron")!=0){    //YO/20140325 -> changed
	  
	total_source_function = particle.primary_source_function;
	total_source_function += particle.secondary_source_function;

	/*
	if(galdef.rm_nearbysource==1 && strcmp(particle.name, "primary_electrons")==0){ //YO/20140611
	  mk_primary_electron_table(particle); //YO/20140616
	  //primary_source_function_sub = particle.primary_source_function;
	}
	*/  
	...
	//check primary, secondary and total source for each nuclei
	if(galdef.check_num==320){ //YO/20140320
	  for (ix = 0; ix < particle.n_xgrid; ++ix) {
	    for (iy = 0; iy < particle.n_ygrid; ++iy) {
	      for (iz = 0; iz < particle.n_zgrid; ++iz) {
		for (ip = 0; ip < particle.n_pgrid; ++ip) {
		  //if(sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5) + particle.y[iy]*particle.y[iy] +particle.z[iz]*particle.z[iz]) <= galdef.near_distance){
		    cerr << "x= " << particle.x[ix] << " y= " << particle.y[iy] << " z= " << particle.z[iz] 
			 << " primary_source= " << particle.primary_source_function.d3[ix][iy][iz].s[ip] 
			 << " secondary_source= " << particle.secondary_source_function.d3[ix][iy][iz].s[ip] 
			 << " total_source= " << total_source_function.d3[ix][iy][iz].s[ip] << endl;
		    //}
		}
	      }
	    }
	...
	if(strcmp(particle.name,"primary_TD_positron")==0 ||  //YO/20141224
	   strcmp(particle.name,"primary_TD_electron")==0){	
	  if(galdef.random_age==1){
	    nrept = particle.m_source_age/(dt/year2sec);
	    //cerr << "nerpt=" << nrept << endl;
	    //for(int i=1; i<=nrept; i++){
	    particle.m_final_time = nrept*(dt/year2sec);
	      //}
	    //cerr << "finaltime = " << particle.m_final_time << endl;
	  }
	}
	...
	for (int irept = 1; irept <= nrept; ++irept) {
	  ...
	  particle.m_dt = dt/year2sec; //YO/20140325

	  if(galdef.check_num==305){
	    cerr << "propagating time = " << particle.m_step_total_time << "yr dt = " << particle.m_dt << "yr" << endl;
	  } //YO/20140325

	  //time dependent secondary source function //YO/20140325 -> 20140408 changed
	  if(strcmp(particle.name, "primary_TD_positron")==0 ||
	     strcmp(particle.name, "primary_TD_electron")==0){
	    gen_TD_secondary_source(particle);
	    ...
	    if(galdef.check_num==521){ //YO/20140521
	      for (ix = 0; ix < particle.n_xgrid; ++ix) {
		for (iy = 0; iy < particle.n_ygrid; ++iy) {
		  for (iz = 0; iz < particle.n_zgrid; ++iz) {
		    for (ip = 0; ip < particle.n_pgrid; ++ip) {
	  ...
	  //remove nearby source contribution from primary electrons  //YO/20140616
	  //if(galdef.rm_nearbysource==1 && strcmp(particle.name, "primary_electrons")==0){ //SB 20161008 takes care of all nuclei including electrons
	  if(galdef.rm_nearbysource==1){
	    if((particle.m_final_time-particle.m_step_total_time)<=galdef.near_age){
	      // cerr << " >>>>>  remove nearby source from " << particle.m_final_time-particle.m_step_total_time << "yr" << endl;
	      for(int ix=0; ix<particle.n_xgrid; ix++)
                for(int iy=0; iy<particle.n_ygrid; iy++)
                  for(int iz=0; iz<particle.n_zgrid; iz++)                     
		    if(galdef.near_distance2!=0 && galdef.near_distance3!=0 && galdef.near_distance4!=0){
	                    spdistance = sqrt(particle.x[ix]*particle.x[ix]+
	            	    particle.y[iy]*particle.y[iy]);//+ particle.z[iz]*particle.z[iz]); no need of z in galactic plane SB20163112                 
	                    //cout<< "!!!!going good!!!!" ; 
		    	    //cout<< spdistance ;
	            	    fraction  = ((spdistance-galdef.near_distance)*(spdistance-galdef.near_distance))/(2*0.36);
		    	    fraction1 = ((spdistance-galdef.near_distance2)*(spdistance-galdef.near_distance2))/(2*0.36);
			    fraction2 = ((spdistance-galdef.near_distance3)*(spdistance-galdef.near_distance3))/(2*0.36);
			    fraction3 = ((spdistance-galdef.near_distance4)*(spdistance-galdef.near_distance4))/(2*0.36);
		    	    newfunction = exp(-fraction     ) +exp(-fraction1)+ exp(-fraction2)+ exp(-fraction3);
		    	    //cout<<"expofunction is"<< newfunction<< endl;
                      for(int ip=0; ip<particle.n_pgrid; ip++){
                        particle.nearby_source_function.d3[ix][iy][iz].s[ip] = (particle.primary_source_function.d3[ix][iy][iz].s[ip])*newfunction;
                      }//ip for loop 
                    }//if distance
	    }// if galdef timestep
	    
	    total_source_function=particle.nearby_source_function+particle.secondary_source_function;
          }//nearby source  
	//   if(galdef.rm_nearbysource=1 ){ //YO/20140618
	//      if((particle.m_final_time-particle.m_step_total_time)<=galdef.near_age){\\SB 20161008
	        // cerr << " >>>>>  remove nearby source from " << particle.m_final_time-particle.m_step_total_time << "yr" << endl;	
	//        for (ix = 0; ix < particle.n_xgrid; ++ix) 
	//	  for (iy = 0; iy < particle.n_ygrid; ++iy) 
	//	    for (iz = 0; iz < particle.n_zgrid; ++iz) 
	//	      if(sqrt( (particle.x[ix]-8.5)*(particle.x[ix]-8.5) +
	//		     particle.y[iy]*particle.y[iy] + 
	//		     particle.z[iz]*particle.z[iz])<=galdef.near_distance){	
	//	        for (ip = 0; ip < particle.n_pgrid; ++ip) {
	//		  particle.primary_source_function.d3[ix][iy][iz].s[ip] = 0;
	  ...
	  //remove nearby source contribution from primary electrons  //YO/20140611
	  /*if(galdef.rm_nearbysource==1 && strcmp(particle.name, "primary_electrons")==0){
	    particle.primary_source_function=primary_source_function_sub;
	    total_source_function=particle.primary_source_function+particle.secondary_source_function;
	    if((particle.m_final_time-particle.m_step_total_time)<=galdef.near_age){
	      for(ix=0; ix<particle.n_xgrid; ++ix){
		for(iy=0; iy<particle.n_ygrid; ++iy){
	  ...
	  cout<<"total time :" << particle.m_step_total_time << endl;
	  particle.m_step_total_time += dt/year2sec; //YO/20140611
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz,ix,iy,ip)
	...
	if(galdef.check_num==305){
	  cerr << "finally total time = " << particle.m_final_time << endl;
	  }*/ //YO/20140325
    ...
    total_source_function.delete_array();
    //primary_source_function_sub.delete_array(); //YO/20140612
    Work.delete_array();
