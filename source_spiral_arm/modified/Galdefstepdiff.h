...
class Galdef {
 ...
 public:
  ...
  double z_step;                        // for thickness over which we apply D_fact    
  ...
  double D_fact;                        // factor that will be multiplied with D0_xx for particles far away from galactic plane SB20170323
  double diff_TeV;                      // 1=use other diffusion coefficient in TeV region YO/20140511                                                                              
  double D_rigid_br_TeV;                // YO/20140511                                                                              
  double D0_xx_TeV;                     // YO/20140511                                                                                                                              
  double D_g_TeV;                       // YO/20140511
...
//  int CO_survey;                        // CO survey : 8=original 8 rings                 9= 9 rings all-sky  AWS20050913
  ...
  //original parameters
  double DM_mass;                //YO/20151012
  double DM_boostfactor;         //YO/20151012
  double DM_sigmav;              //YO/20151012
  int DM_model;                  //YO/20151012


  int primary_TD_positron;     // 1=propagate positron produced by time_dependent injection Y0/20140220->20140408 changed
  int primary_TD_electron;     // 1=propagate electron produced by time_dependent injection Y0/20140408
  double t_TDmax;              // parameter for test calculation YO/20140310
  double t_TDmin;              // parameter for test calculation YO/20140310
  double source_age;           // age of electron source(pulsar,SNR) [yr] YO/20140325

  int TD_SNR_model;            // 1=electron burst-like injection (Ref:Astro.J T.Kobayashi 2004) YO/20140425
                               // 2=electron exponential decay injection (Ref:arXiv0903.3782v2 N.Kawanaka 2009) YO/20140425

  int random_coordinate;       // 1=calculate single source contribution with random distance YO/20141213
  double rand_max,rand_min;    // maximum and minimum of distance range YO/20141213

  int random_age;              // 1=calculate single source age with random parameter YO/20141224
  double rant_max,rant_min;    // maximum and minimum of age range YO/20141224

  double SNR_x,SNR_y,SNR_z;    // in TDmodel1(burst-like), SNR corrdinates (x,y,z) YO/20141117
  double total_energy;         // electron total energy in SNR [erg] YO/20141117
  //double power_law;            // power law index in SNR source YO/20141117
  double power_law_1;
  double power_law_2;
  double power_law_br;
  int    inj_energy_cutoff;    // 1=considering energy cutoff in injection spectrum YO/20151211
  double energy_cutoff;        // energy cut-off in SNR source YO/20141117

  double longitude,latitude;   // galactic longitude & latitude of single source YO/20141213
  double distance;             // distance of single source from earth YO/20141213
  
  int rm_nearbysource;         // 1=exclude nearby sources (only 3D mode) YO/20140314
  double near_distance;        // exclude CR sources in a distance below near_distance from the earth YO/20140314
  double near_distance2;       // near_distance1 and near_distance2 are the distance of two spiral arms... SB/20161113
  double near_distance3;       // more arms 
  double near_distance4;       // 4arms
  double near_age;             // exclude young CR sources YO/20140612
  //int exponfactor;              // define the slope of the exponential function SB/20161110
  int check_num;               // check calculation using cerr YO/20140325
