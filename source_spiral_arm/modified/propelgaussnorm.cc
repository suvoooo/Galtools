...
int Galprop::propel(Particle& particle) {
  ...
  double fraction1; //SB20161111
  double fraction2; //SB20161111
	    ...
	    if((particle.m_final_time-particle.m_step_total_time)<=galdef.near_age){
	      ...
	      for(int ix=0; ix<particle.n_xgrid; ix++)
		    ...
		    if(sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5)+
                            ...
                            particle.z[iz]*particle.z[iz])<=(galdef.near_distance -sigma) ){ // && sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5)+
                           // particle.y[iy]*particle.y[iy]+
                           // particle.z[iz]*particle.z[iz])<=(galdef.near_distance+sigma) && sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5)+
                           // particle.y[iy]*particle.y[iy]+
                           // particle.z[iz]*particle.z[iz])>(galdef.near_distance2-sigma) && sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5)+
                           // particle.y[iy]*particle.y[iy]+
                           // particle.z[iz]*particle.z[iz])<=(galdef.near_distance2+sigma) ){
			    //cerr<< "!!!! nearby distance-" << sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5)+particle.y[iy]*particle.y[iy]+particle.z[iz]*particle.z[iz]) << "kpc" << endl;
			    spdistance = sqrt((particle.x[ix]-8.5)*(particle.x[ix]-8.5)+
			    particle.y[iy]*particle.y[iy]+
			    particle.z[iz]*particle.z[iz]);
			    cout<< "!!!!going good!!!!" ; 
			    cout<< spdistance ;
			    fraction1 = ((spdistance-galdef.near_distance)*(spdistance-galdef.near_distance))/(2*(1*1));
			    fraction2 = ((spdistance-galdef.near_distance2)*(spdistance-galdef.near_distance2))/(2*(1*1)); // sigma values can be 1kpc, 0.7kpc, 0.3kpc as try cases...
			    newfunction = exp(-fraction1     )+ exp(-fraction2 );
			    cout<<"expofunction is"<< newfunction<< endl;
                      for(int ip=0; ip<particle.n_pgrid; ip++){
                        particle.primary_source_function.d3[ix][iy][iz].s[ip] = (particle.primary_source_function.d3[ix][iy][iz].s[ip])*newfunction;
                      }//ip for loop 
                    }//if distance
	    }// if galdef timestep
