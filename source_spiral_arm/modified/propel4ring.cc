	...
	for (int irept = 1; irept <= nrept; ++irept) {
	  ...
	  if(galdef.rm_nearbysource==1){
	    if((particle.m_final_time-particle.m_step_total_time)<=galdef.near_age){
	      // cerr << " >>>>>  remove nearby source from " << particle.m_final_time-particle.m_step_total_time << "yr" << endl;  \\ try removing z
	      for(int ix=0; ix<particle.n_xgrid; ix++)
		    ...
		    if(galdef.near_distance2!=0 && galdef.near_distance3!=0 && galdef.near_distance4!=0){
		    	    ...
		    	    particle.z[iz]*particle.z[iz]);                 
	                    //cout<< "!!!!going good!!!!" ; 
		    	    //cout<< spdistance ;
	            	    fraction = ((spdistance-galdef.near_distance)*(spdistance-galdef.near_distance))/(2*1.69);
		    	    fraction1 = ((spdistance-galdef.near_distance2)*(spdistance-galdef.near_distance2))/(2*1.69);
			    fraction2 = ((spdistance-galdef.near_distance3)*(spdistance-galdef.near_distance3))/(2*1.69);
			    fraction3 = ((spdistance-galdef.near_distance4)*(spdistance-galdef.near_distance4))/(2*1.69);
