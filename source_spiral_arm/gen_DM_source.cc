...
int Galprop::gen_DM_annihilation(Particle &particle)
{
 ...
 // define the spectra of annihilation products: positrons, electrons, antiprotons
   ...
   if(strcmp(particle.name,"primary_DM_positron")==0){
     ifstream fin;
     const string DMpfn = configure.fGlobalDataPath + "/" + galdef.DM_positron_filename;
	cout<<"configure.fGlobalDataPath"<<endl;     
	fin.open(DMpfn.c_str(),ios::in); 
   ...
   if(strcmp(particle.name,"primary_DM_electron")==0){
     ifstream fin;
   const string DMefn = configure.fGlobalDataPath + "/" + galdef.DM_electron_filename;// line that guides the name of the GALDEF file for electrons
   fin.open(DMefn.c_str(),ios::in);
