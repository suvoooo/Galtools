...
//   z_step = galdef.z_step;
//   D_fact = galdef.D_fact; 	
	 ...
	 if(n_spatial_dimensions==2)
	   {
	     ...
	     //TeV region  Ref:T.Kobayashi,2004,ApJ:601,340-351  YO/20140511  
             if(galdef.diff_TeV==1 && particle.rigidity[ip]>=galdef.D_rigid_br_TeV){
               particle.Dxx.d2[ir][iz].s[ip] = particle.beta[ip]*galdef.D0_xx_TeV;
               if(galdef.diff_reacc<0) particle.Dxx.d2[ir][iz].s[ip] = pow(particle.beta[ip],galdef.diff_reacc)*galdef.D0_xx_TeV;
               particle.Dxx.d2[ir][iz].s[ip]*= pow(particle.rigidity[ip]/1.e6/*galdef.D_rigid_br_TeV*/, galdef.D_g_TeV); //YO20140513

	       if(galdef.check_num==512){
		 cerr << "rigidity = " << particle.rigidity[ip] << " Dxx = " << particle.Dxx.d2[ir][iz].s[ip] << endl;
	       }
	 ...
	 if(n_spatial_dimensions==3)
	   {
	     ...
	     //if (iz > galdef.z_step) particle.Dxx.d3[ix][iy][iz].s[ip] = pow(particle.beta[ip],galdef.diff_reacc) *galdef.D0_xx*galdef.D_fact;//SB20170323	
	     ...
	     //TeV region  YO/20140511
	     if(galdef.diff_TeV==1 && particle.rigidity[ip]>=galdef.D_rigid_br_TeV){
	       particle.Dxx.d3[ix][iy][iz].s[ip] = particle.beta[ip]*galdef.D0_xx_TeV;
	       if(galdef.diff_reacc<0) particle.Dxx.d3[ix][iy][iz].s[ip] = pow(particle.beta[ip],galdef.diff_reacc)*galdef.D0_xx_TeV;
	       particle.Dxx.d3[ix][iy][iz].s[ip]*= pow(particle.rigidity[ip]/1.e6/*galdef.D_rigid_br_TeV*/, galdef.D_g_TeV); //YO20140513
	     }
	   }
       }
     return 0;
